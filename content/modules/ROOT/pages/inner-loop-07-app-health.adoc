...
There are three types of health probes available in OpenShift: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/building_applications/application-health[startup, readiness and liveness probes^].
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], under the Workloads menu
`*click on 'Topology' -> the '(D) inventory-coolstore' Deployment -> 'Resources' -> the '(P) inventory-coolstore-xxxxxxxxxx-xxxxx' pod*`

image::openshift-inventory-pod.png[OpenShift Inventory Pod,700,link=self,window=blank]

On the pod details page, `*click on 'Actions' -> 'Delete Pod' -> 'Delete'*`

image::openshift-inventory-delete-pod.png[OpenShift Inventory Delete Pod,700,link=self,window=blank]

A new instance (pod) will be redeployed very quickly. Once deleted `*try to access your link:http://inventory-coolstore-{DEV_PROJECT}.{OPENSHIFT_APPS_DOMAIN}[Inventory Service^]*`.
...
Let's imagine you have traffic coming into the _Inventory Service_. We can do that with a simple script.
...
for i in {1..60}
do
    if [ $(curl -s -w "%\{http_code}" -o /dev/null http://inventory-coolstore.{DEV_PROJECT}.svc:8080/api/inventory/329299) == "200" ]
    then
...
image::che-inventory-traffic.png[Che - Catalog Traffic OK,500,link=self,window=blank]

Now let's scale out your _Inventory Service_ to 2 instances.

In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], under the Workloads menu,
`*click on 'Topology' -> '(D) inventory-coolstore' -> 'Details' then click once on the up arrow
on the right side of the pod ring*`.

image::openshift-scale-out-inventory.png[OpenShift Scale Out Catalog,700,link=self,window=blank]

You should see the 2 instances (pods) running.
Now, `*switch back to your link:{WORKSPACE_URL}[Workspace^] and check the output of the 'Inventory - Generate Traffic' task*`.

image::che-inventory-traffic-ko.png[Che - Catalog Traffic KO,500,link=self,window=blank]
...
TIP: This is a race condition based on that startup speed, sometimes this script won't fail, if that is the case scale the pods back to 1 instance and try again.
...
First, scale down your _Inventory Service_ to 1 instance. In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], under the Workloads menu,
`*click on 'Topology' -> '(D) inventory-coolstore' -> 'Details' then click once on the down arrow
on the right side of the pod ring*`.

Now let's go fix some of these problems.

[#configuring_liveness]
== Configuring Liveness Probes

https://quarkus.io/guides/smallrye-health[SmallRye Health^] is a Quarkus extension which utilizes the MicroProfile Health specification.
...
In your link:{WORKSPACE_URL}[Workspace^], `*edit the '/projects/workshop/labs/inventory-quarkus/pom.xml' file*`:
...
[source,shell,subs="attributes+",role=copypaste]
----
curl -w "\n" http://inventory-coolstore.{DEV_PROJECT}.svc:8080/q/health
----
...
You can check this in the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], under the Workloads menu,
`*click on 'Topology' -> '(D) inventory-coolstore' -> 'Actions' -> 'Edit Health Checks'*`.

image::openshift-inventory-edit-health.png[Che - Inventory Add Health Check,700,link=self,window=blank]
...
image::openshift-inventory-view-health.png[Che - Inventory View Health Check,700,link=self,window=blank]
...
`*Generate traffic to Inventory Service*` and then, in the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^],
`*scale out the Inventory Service to 2 instances (pods)*`

In your link:{WORKSPACE_URL}[Workspace^], `*check the output of the 'Inventory - Generate Traffic' task*`.
...
[#catalog_services_probes]
== Catalog Services Probes

https://docs.spring.io/spring-boot/reference/actuator/index.html[Spring Boot Actuator^] is a
...
In your link:{WORKSPACE_URL}[Workspace^], in the *terminal* window,
...
image::openshift-catalog-edit-health.png[Che - Catalog Add Health Check,700,link=self,window=blank]
...
Sometimes, some (legacy) applications might need extra time for their first initialization.
In such cases, setting a longer liveness interval might compromise the main benefit of this probe, that is providing
the fast response to stuck states.

**Startup probes** are useful to cover this worst case startup time.
...
In your link:{WORKSPACE_URL}[Workspace^], `*click on 'Terminal' -> 'Run Task...' -> 'devfile: Probes - Configure Gateway & Web'*`
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], from the Workloads menu, `*click on 'Topology' -> any Deployment (D) and click on the associated Pod (P)*`

In the pod `*Metrics*` tab, you can see a more detailed view of the pod consumption.

image::openshift-pod-details.png[OpenShift Pod Details,740,link=self,window=blank]
