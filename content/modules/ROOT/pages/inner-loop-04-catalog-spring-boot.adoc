...
image::coolstore-arch-catalog-spring-boot.png[CoolStore Architecture,400,link=self,window=blank]
...
[sidebar]
--
image::spring-boot-logo.png[Spring Boot,400,link=self,window=blank]
...
image::springboot-catalog-project.png[Catalog Project,400,link=self,window=blank]
...
image::springboot-catalog-arch.png[Catalog RESTful Service,640,link=self,window=blank]

[#create_domain_model]
== Create the Domain Model

In your link:{WORKSPACE_URL}[Workspace^], `*create the 'src/main/java/com/redhat/cloudnative/catalog/Product.java' file*`
...
persistence stores. https://docs.spring.io/spring-data/jpa/reference/repositories/core-concepts.html[Repository and its sub-interfaces^]
...
https://docs.spring.io/spring-boot/reference/using/spring-beans-and-dependency-injection.html[**@Autowired**^] annotation.
...
image::che-open-9000-link.png[Che - Open Link,500,link=self,window=blank]

Your browser will be directed to **your Catalog Service running inside your Workspace**.

image::catalog-service.png[Catalog Service,500,link=self,window=blank]

[NOTE]
====
If the browser shows that the preview is not available, the service has not finished starting yet. Wait a few seconds and refresh the page.

image::che-preview-na.png[Che - Preview Not Available,500,link=self,window=blank]
====
...
It's time to deploy your service on OpenShift. This time we are going to use Eclipse https://eclipse.dev/jkube/docs/openshift-maven-plugin/[JKube^] to define the
...
NOTE: The *pom.xml* also sets the property `jkube.build.switchToDeployment` to `true`, so JKube creates a Kubernetes `Deployment` instead of the deprecated OpenShift `DeploymentConfig`.
...
As you did previously, `*Deploy a new Component to the OpenShift cluster*`
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], `*click on the 'Open URL' icon of the Catalog Service*`

image::openshift-catalog-topology.png[OpenShift - Catalog Topology,700,link=self,window=blank]
