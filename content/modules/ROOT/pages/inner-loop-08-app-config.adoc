...
OpenShift provides a mechanism called https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/nodes/working-with-pods#configmaps[**ConfigMaps**^]
...
Let's create a https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/building_applications/creating-applications[MariaDB database^]
for the **Inventory Service** using the MariaDB template that is provided for this workshop:

[TIP]
====
https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/building_applications/creating-applications[OpenShift Templates^] use YAML/JSON to compose
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], `*click on the 'Ecosystem' menu, and choose 'Software Catalog'*`
...
image::openshift-add-database.png[OpenShift - Add database,800,link=self,window=blank]

`*Select 'Coolstore MariaDB (Ephemeral)' and click on 'Instantiate Template'*`

Then, enter the following information:

.Inventory Database - Coolstore MariaDB (Ephemeral)
[%header,cols=2*]
|===
|Parameter
|Value
...
`*Click on 'Create' button*`, shortly a MariaDB database pod should be created:

image::openshift-inventory-mariadb-topology.png[OpenShift - Inventory MariaDB,700,link=self,window=blank]

Now select again from the `*'Software Catalog'*`, this time choose `*'Coolstore PostgreSQL (Ephemeral)' and click on 'Instantiate Template'*`
to create the Catalog Database.

Then, enter the following information:

.Catalog Database - Coolstore PostgreSQL (Ephemeral)
[%header,cols=2*]
|===
|Parameter
|Value
...
image::openshift-catalog-postgresql-topology.png[OpenShift - Catalog PostgreSQL,700,link=self,window=blank]

Both templates create a *Deployment*, a *Service* and a *Secret* named after the database service.
...
Since you do want our applications to discover the config maps inside the **{DEV_PROJECT}** project, you need `*to grant permission to the Service Account to access the OpenShift REST API*` and find the config maps.

In your link:{WORKSPACE_URL}[Workspace^], `*execute the following command in the terminal window*`:
...
configuration is however using an https://quarkus.io/guides/config-reference[application.properties^]
which you have already packaged within the Inventory Maven project.

In Quarkus, the database kind is a build time property and cannot be overridden at runtime. So as you are going to change the database
technology, you need to change the 'quarkus.datasource.db-kind' parameter
in **/projects/workshop/labs/inventory-quarkus/src/main/resources/application.properties** and rebuild the application.

In your link:{WORKSPACE_URL}[Workspace^], `*edit the '/projects/workshop/labs/inventory-quarkus/pom.xml' file and add the
'Kubernetes Config' and 'JDBC Driver - MariaDB' dependencies*`
...
Then `*add the '%prod.quarkus.datasource.db-kind' parameter and the Kubernetes Config parameters in
...
%prod.quarkus.datasource.db-version=10.3
----
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], from the `*Workloads*` menu
`*click on 'ConfigMaps' then click on the 'Create ConfigMap' button*`.

image::openshift-create-configmap.png[Che - OpenShift Create Config Map,900,link=self,window=blank]
...
data:
  application.properties: |-
    quarkus.datasource.jdbc.url=jdbc:mariadb://inventory-mariadb.{DEV_PROJECT}.svc:3306/inventorydb
...
[source,shell,subs="attributes+",role=copypaste]
----
oc delete pod -l component=inventory -n {DEV_PROJECT}
----

[#check_inventory_database]
...
[source,shell,subs="attributes+",role=copypaste]
----
oc rsh -n {DEV_PROJECT} deploymentconfig/inventory-mariadb
----
...
https://docs.spring.io/spring-boot/reference/features/external-config.html[overridden and overlaid via multiple mechanisms^].
...
data:
  application.properties: |-
    spring.datasource.url=jdbc:postgresql://catalog-postgresql.{DEV_PROJECT}.svc:5432/catalogdb
...
The https://github.com/spring-cloud/spring-cloud-kubernetes[Spring Cloud Kubernetes^] plug-in implements
...
[source,shell,subs="attributes+",role=copypaste]
----
oc delete pod -l component=catalog -n {DEV_PROJECT}
----
...
[source,shell,subs="attributes+",role=copypaste]
----
oc logs deployment/catalog-coolstore -n {DEV_PROJECT} | grep hibernate.dialect
----
...
[source,shell]
----
... INFO 1 --- [           main] org.hibernate.dialect.Dialect            : HHH000400: Using dialect: org.hibernate.dialect.PostgreSQL95Dialect
----
...
[source,shell,subs="attributes+",role=copypaste]
----
oc rsh -n {DEV_PROJECT} deploymentconfig/catalog-postgresql
----
...
called https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/nodes/working-with-pods#nodes-pods-secrets[**Secrets**^]. Secret objects act and are used
...
[source,shell,subs="attributes+",role=copypaste]
----
oc describe secret catalog-postgresql -n {DEV_PROJECT}
----
...
Namespace:    {DEV_PROJECT}
Labels:       app=catalog-postgresql
              template=coolstore-postgresql
              ...
Annotations:  <none>
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], from the `*Workloads*` `*Topology*` menu,
`*click on the '(D) catalog-postgresql' Deployment, then on its name 'catalog-postgresql' in the side panel -> 'Environment'*`. Notice the values
from the secret are defined as environment variables on the deployment:

image::config-psql-secret.png[Secrets as Env Vars,900,link=self,window=blank]
