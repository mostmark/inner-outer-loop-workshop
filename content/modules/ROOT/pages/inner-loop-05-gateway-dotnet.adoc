...
image::coolstore-arch-gateway-dotnet.png[CoolStore Architecture,400,link=self,window=blank]
...
[sidebar]
--
image::240px-NET_Core_Logo.png[.NET,240,link=self,window=blank]

https://learn.microsoft.com/en-us/dotnet/core/introduction[.NET (Core)^] is a
free, open-source development platform for building many kinds of apps, from web, serverless, mobile, desktop and console apps.
...
source, https://github.com/dotnet/runtime/blob/main/LICENSE.TXT[using MIT and Apache 2 licenses^]. .NET is a project of the https://dotnetfoundation.org/[.NET Foundation^].

pass:[.NET] supports a number of programming languages and development environments, but today we will be looking at C# inside OpenShift Dev Spaces.
...
image::dotnet-gateway-project.png[Gateway Project,340,link=self,window=blank]
...
oc new-build dotnet:9.0 --name gateway-coolstore  \
  ...
  --env DOTNET_STARTUP_PROJECT=app.csproj --binary=true -n {DEV_PROJECT}
oc start-build gateway-coolstore --from-dir=. -w -n {DEV_PROJECT}

oc new-app gateway-coolstore:latest --name gateway-coolstore  --labels=app=coolstore,app.kubernetes.io/instance=gateway,app.kubernetes.io/part-of=coolstore,app.kubernetes.io/name=gateway,app.openshift.io/runtime=dotnet,component=gateway -n {DEV_PROJECT}

oc expose svc gateway-coolstore -n {DEV_PROJECT}
----
...
In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^], `*click on the 'Open URL' icon of the Gateway Service*`

image::openshift-gateway-topology.png[OpenShift - Gateway Topology,700,link=self,window=blank]

Your browser will be redirected to **your Gateway Service running on OpenShift**.

image::gateway-service.png[Gateway Service,500,link=self,window=blank]
...
public static void Config()
{
    try
    {
        ...
        // set up the Http connection pools
...
You can try this simple name service discovery for yourself in the Gateway service pod. In the link:{CONSOLE_URL}/topology/ns/{DEV_PROJECT}[OpenShift Web Console^],
`*select the Gateway service and then the running Pod.*`

image::openshift-gateway-pod.png[Gateway Service,500,link=self,window=blank]

You can test the connectivity by `*selecting the Pod Terminal*` and by `*executing these shell commands in the terminal window:*`

image::openshift-gateway-pod-terminal.png[Gateway Service,500,link=self,window=blank]
